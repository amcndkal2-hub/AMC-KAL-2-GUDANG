  ...
  tableBody.innerHTML = rabList.map((rab, index) => {
    ...
    <tr class="hover:bg-gray-50 transition-colors border-b" style="animation: slideIn 0.3s ease-out ${index * 0.05}s both; height: 48px;">
      ...
      <td class="px-2 py-2 border text-center align-middle" style="background: white; width: 140px; min-width: 140px; max-width: 140px;">
        ${rab.jenis_rab === 'Pembelian Langsung'
          ? renderRABStatusDropdown(rab)
...
function renderRABStatusDropdown(rab) {
  ...
  return `
    <select onchange="updateRABStatusManual(${rab.id}, this.value, this)"
      class="w-full px-2 py-1 rounded-lg text-xs font-semibold text-center cursor-pointer ${colorClass} focus:outline-none focus:ring-1 focus:ring-blue-400"
...
// Update status RAB Pembelian Langsung via API
async function updateRABStatusManual(rabId, newStatus, selectEl) {
  ...
  try {
    ...
    if (response.ok) {
      ...
      const newColor = colorMap[newStatus] || colorMap['Draft']
      selectEl.className += ' ' + newColor
    ...
    selectEl.value = prevValue
    showNotification('Gagal mengubah status (error jaringan)', 'error')
  } finally {
